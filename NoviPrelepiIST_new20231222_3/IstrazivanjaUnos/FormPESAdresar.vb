...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FormPESAdresar))
        ...
        Me.Panel3.Location = New System.Drawing.Point(41, 1011)
        ...
        Me.PanelZakazivanjeStana.Location = New System.Drawing.Point(373, 512)
        ...
        Me.rb1.Name = "rb1"
        Me.rb1.Size = New System.Drawing.Size(798, 17)
        Me.rb1.TabIndex = 0
        Me.rb1.TabStop = True
        Me.rb1.Text = "1 - Ostavljeno obaveštenje o dolasku popisivača, obrazac KP-11 (trenutno nema nik" &
    "oga u stanu/zatečeno lice ne može da da podatke)"
        ...
        Me.dtpDatumDolaska.Items.AddRange(New Object() {"02.12.2022", "03.12.2022", "04.12.2022", "05.12.2022", "06.12.2022", "07.12.2022", "08.12.2022", "09.12.2022", "10.12.2022", "11.12.2022", "", ""})
        ...
        Me.dtpZakazanoDatum.Items.AddRange(New Object() {"02.12.2022", "03.12.2022", "04.12.2022", "05.12.2022", "06.12.2022", "07.12.2022", "08.12.2022", "09.12.2022", "10.12.2022", "11.12.2022", "", ""})
        ...
        Me.PanelIzmenaUL.Location = New System.Drawing.Point(273, 516)
    ...
    "o naziv postojećih ulica)"
        ...
        Me.tboxObjasnjenjeUklanjanja.MaxLength = 250
...
#End Region
    ...
    Private Sub FormDnevnikAnketiranja_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        PanelPKDorada.Location = Panel3.Location
    ...
    Sub Dgv3CellContentClick(ByVal erowindex As Integer, ByVal eColumnIndex As Integer)
        ...
        If erowindex >= 0 Then
            ...
            If DGV3.Columns(eColumnIndex).DisplayIndex = DGV3.Columns("OOPENSTAN").DisplayIndex Then
                ...
                Dim zakljucanOdmahSve As Boolean = False
                If DGV3.Rows(erowindex).Cells("UStanuVanStana").Value.ToString.Trim = "Ne živi u stanu na današnji dan" Then
                    podtabela = "spisaklicaostali"
                Else
    ...
    Sub refreshDGV1(Optional ByVal ulicaKBroj As String = "", Optional ByVal status As String = "", Optional ByVal NoviKbrojTeren As String = "", Optional ByVal NapomenaOKucnomBroju As String = "", Optional ponistisort As Boolean = False)
        ...
        If atbUlica.sselectedValue <> "" AndAlso atbUlica.sselectedValue <> "000000000000" Then
            str = "select * from (select distinct" +
            ...
            " a.ulicaKBroj," + nvrd +
            " (case when isnull(b.adresateren,0)=4 then N'Ne postoji zgrada na ovoj adresi' else (b.noviNazivUliceTeren+' '+b.NoviKbrojTeren) end) as NoviKbrojTeren, " + nvrd +
            " N'🏠' as OOpen1," + nvrd +
            ...
            " a.kbroj,a.ulica,a.longituda,a.latituda," + nvrd + '" a.krug,a.kbroj,a.ulica,a.longituda,a.latituda," + nvrd +
            ...
            " from (select * from adresar where ulicaKBroj like N'%" + ulicaKBroj + "%' and IDANKETAR=N'" + kosam + "') a left join " + nvrd +
        ...
        ElseIf atbUlica.sselectedValue = "000000000000" Then
            str = "select * from (select distinct" + nvrd +
            " (case when isnull(b.adresateren,0)=0 then ' '" + nvrd +
            " when isnull(b.adresateren,0) not in (0,4) and isnull(vrstazgrade,0)=0 then '0' " + nvrd +
            " when isnull(b.adresateren,0) not in (0,4) and isnull(vrstazgrade,0) not in (5,6) " + nvrd +
            " and isnull(b.brojstanova,9999)<> (select count(*) from p2_stan a where a.mbops=b.mbops and a.kucni_broj_idCD=b.kucni_broj_idCD " + nvrd +
        ...
        Else
           ...
           " (case when isnull(b.adresateren,0) not in (0,4) And isnull(vrstazgrade,0) in (0,1,2,3,4) And isnull(b.brojstanova,9999) Not in (0,9999) then" + nvrd +
            ...
            " case when isnull(b.BrojStanova,'')='' then '' else N'🏘' end as DA," + nvrd +
             " N'🖉' as EEdit,b.NapomenaOKucnomBroju, ' ' as praznoB," + nvrd +
            " a.ulica,a.kbroj,a.longituda,a.latituda," + nvrd + '" a.krug,a.ulica,a.kbroj,a.longituda,a.latituda," + nvrd +
        ...
        DGV1.Columns("BrojZakP11Stanova").HeaderText = "ostavljen KP11"
        ...
        DGV1.Columns("BrojvracenihStanova").HeaderText = "stanovi vraćeni na doradu"
    ...
    Private Function PrikaziZavrsetakKruga()
        ...
        Dim Str As String = "select * from (select distinct" + nvrd +
            " (case when isnull(b.adresateren,0)=0 then ' '" + nvrd +
            " when isnull(b.adresateren,0) not in (0,4) and isnull(vrstazgrade,0)=0 then '0' " + nvrd +
            " when isnull(b.adresateren,0) not in (0,4) and isnull(vrstazgrade,0) not in (5,6) " + nvrd +
            " and isnull(b.brojstanova,9999)<> (select count(*) from p2_stan a where a.mbops=b.mbops and a.kucni_broj_idCD=b.kucni_broj_idCD " + nvrd +
                    ...
                    Select Case opc2
                        Case "Yes"
                            ...
                            Else
                                str = "select '000' "
                            End If
                            Dim ok1 As String = izvrsiSQL(str, Me.DBConnectionString)
                            If ok1 <> "ok" Then
                                ISTMessageBox.Show(str + nvrd + nvrd + "is not executed! ER58", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            End If
                            Call Reset()
                            If atbOps.Text = "" Then
                                atbOps.Focus()
    ...
    Function dajSQLzaDGV2(ByVal pv As String, ByVal dv As String, Optional ByVal tv As String = "", Optional datumzakazano As String = "", Optional ByVal status As String = "", Optional ByVal ulicakbroj As String = "", Optional ByVal novikbroj As String = "") As String
        ...
        Dim str As String = "select * from (select left(a.Krug_idcd,4) as krugZaPrikaz,a.NazivNaselja," + nvrd +
        ...
        " else cast(isnull(a.statusAnketar,'') as varchar(1)) end) as statusAnketar,case when isnull(ooo.kucni_broj_idCD,'')='' then '' else cast(a.RbrStan as char(10)) end as RbrStan, a.ulicaKBroj," + nvrd +
        " (OOO.noviNazivUliceTeren+' '+OOO.NoviKbrojTeren) as NoviKbrojTeren," + nvrd +
        ...
        " (select cast(sum(isnull(BrLica,0)) as varchar(3)) from p2_dom bb where bb.mbops=a.mbops and bb.Krug_idcd=a.Krug_idcd And bb.kucni_broj_idCD=a.kucni_broj_idCD And bb.rbrstan=a.rbrstan and isnull(Izabranodom,0)=1) +'/'+" + nvrd +
        ...
        " a.kucni_broj_Deo,a.Krug_idcd,a.idulice,a.kucni_broj_idCD,a.ulica,a.kbroj,a.cati_nacin, " + nvrd + '" a.Krug, a.kucni_broj, a.kucni_broj_Deo,a.Krug_idcd, a.idulice,a.kucni_broj_idCD,a.ulica,a.kbroj,a.cati_nacin, " + nvrd +
        ...
        " and isnull(m.ZakazanoDatum,' ') like N'%" + datumzakazano + "%' and isnull(m.statusanketar,' ') like N'%" + status + "%' and isnull(m.ulicaKBroj,' ') like N'%" + ulicakbroj + "%' and isnull(m.NoviKbrojTeren,' ') like N'%" + novikbroj + "%' order by m.kbroj"
        ' " left join (select nacinanketiranja,naziv from nacinanketiranja where IDLang='" + Me.jezik.Trim + "') NA on a.cati_nacin=NA.nacinanketiranja" +
        Return str
    End Function
    ...
    Function dajSQLzaDGV3(ByVal krug As String, ByVal idulice As String, Optional ByVal kbr_idcd As String = "", Optional ByVal rbrstana As String = "", Optional ByVal fulica As String = "", Optional ByVal fime As String = "", Optional ByVal fjmbg As String = "") As String
        ...
        ' SELECT distinct N'🏠' as OOpen1,
        Dim str As String = "select  " + nvrd +
                            " a.[RbrStan],ulicakbroj,brojstana, UStanuVanStana, " + nvrd +
                            " a.rbrdom,a.rbrp1" + nvrd +
                            " ,ime+' ' +prezime as ImePrezime" + nvrd +
                            " ,[ImeRoditelja],isnull([Pol],'') as pol " + nvrd +
                            ...
                            " ,a.[kucni_broj_idCD],idulice,adr.sourceof_INSERT ,  ime + ' ' + prezime as ImePrezimeCelo" + nvrd +
                            ...
                            "  else cast(isnull(adr.statusAnketar,'') as varchar(1)) end) as statusAnketar from " + nvrd +
                            " (select [RbrStan], N'Živi u stanu na današnji dan' as UStanuVanStana,cast(rbrdom as varchar(5)) as rbrdom,rbrp1,Ime,Prezime,ImeRoditelja,pol,[Dan],[Mesec],[GodRod],rest,[Starost],cast([LiceJe] as varchar(5)) as liceje  " + nvrd +
                            " ,cast([ClanDom] as varchar(5)) as clandom,istnote, " + nvrd +
                            " [MBOPS], [Krug_idcd], [kucni_broj_idCD]" + nvrd +
                            " from" + nvrd +
                            " spisaklica" + nvrd +
                            " union all" + nvrd +
                            " Select [RbrStan],N'Ne živi u stanu na današnji dan' as UStanuVanStana,cast(rbrdom as varchar(5)) as rbrdom,rbrp1,Ime,Prezime,ImeRoditelja,pol,[Dan],[Mesec],[GodRod],rest,[Starost],'/','/',istnote," + nvrd +
                            " [MBOPS], [Krug_idcd], [kucni_broj_idCD]" + nvrd +
                            " From [dbo].[SpisakLicaOstali]  ) a " + nvrd +
                            " left Join p2_dom b on a.[MBOPS]=b.[MBOPS] And a.[Krug_idcd]=b.[Krug_idcd] And a.[kucni_broj_idCD]=b.[kucni_broj_idCD] " + nvrd +
                            ...
                            " ulicakbroj like N'%" + fulica + "%' and (ime + ' ' + prezime like N'%" + fime + "%') and dan+mesec+godrod+rest like '%" + fjmbg + "%'"
        '" and isnull(m.ZakazanoDatum,' ') like N'%" + datumzakazano + "%' and isnull(m.statusanketar,' ') like N'%" + status + "%' and isnull(m.ulicaKBroj,' ') like N'%" + ulicakbroj + "%' and isnull(m.NoviKbrojTeren,' ') like N'%" + novikbroj + "%' order by m.kbroj"
        ' " left join (select nacinanketiranja,naziv from nacinanketiranja where IDLang='" + Me.jezik.Trim + "') NA on a.cati_nacin=NA.nacinanketiranja" +
        Return str
    End Function
    ...
    Function dajSQLzaDGV2Prazan() As String
        ...
        " (select cast(isnull(brojdom,0) as varchar(3)) from p2_stan bb where bb.mbops=a.mbops and bb.Krug_idcd=a.Krug_idcd and bb.kucni_broj_idCD=a.kucni_broj_idCD and bb.rbrstan=a.rbrstan ) +'/'+" + nvrd +
        " cast(isnull((select count(*) from p2_dom xx where xx.mbops=a.mbops and xx.Krug_idcd=a.Krug_idcd and xx.kucni_broj_idCD=a.kucni_broj_idCD and xx.rbrstan=a.rbrstan and isnull(brlica,0)>0 group by rbrstan),0) as varchar(3)) as BrojDom," + nvrd +
        " (select cast(isnull(BrLica,0) as varchar(3)) from p2_dom bb where bb.mbops=a.mbops and bb.Krug_idcd=a.Krug_idcd And bb.kucni_broj_idCD=a.kucni_broj_idCD And bb.rbrstan=a.rbrstan and isnull(Izabranodom,0)=1) +'/'+" + nvrd +
        " cast(isnull((select count(*) from p1 xx where xx.mbops =a.mbops and xx.Krug_idcd=a.Krug_idcd And xx.kucni_broj_idCD=a.kucni_broj_idCD And xx.rbrstan=a.rbrstan And isnull(podatak, 0) > 0 group by rbrstan),0) as varchar(3)) as BrojLica," + nvrd +
        " ' ' as praznoA," +
        " (case when isnull(a.statusAnketar,0)= 0 Then N'📝' else '' end ) as EEdit,DatumDolaska,a.ZakazanoDatum, a.ZakazanoVreme, cast(a.Ishod as nvarchar(1)) +'-'+ ltrim(rtrim(i.naziv)) as nazivishod, " + nvrd +
        " (case when isnull(a.cati_nacin,0)=1 then N'📞' when isnull(a.cati_nacin,0)=2 then N'🏃' else '' end) as nacinanketiranja," + nvrd +
        ...
        " And exists (select * from P2_stan x where a.mbops=x.mbops And a.krug_idcd=x.krug_idcd And a.kucni_broj_idcd=x.kucni_broj_idcd And a.rbrstan=x.rbrstan ) and cast(isnull(a.statusAnketar,'') as varchar(1)) in ('','0','1')" + nvrd +
        " then N'🗑' else '' end) as DeleteIZStana," + nvrd +
        " a.kucni_broj_Deo,a.idulice,a.Krug_idcd,a.kucni_broj_idCD,a.ulica,a.kbroj,a.cati_nacin, " + nvrd + '" a.Krug, a.kucni_broj, a.kucni_broj_Deo,a.Krug_idcd, a.idulice,a.kucni_broj_idCD,a.ulica,a.kbroj,a.cati_nacin, " + nvrd +
        " a.mbops, a.NazivOpstine, a.mbnas, " + nvrd +
        " a.sourceof_INSERT, a.ishod, " + nvrd +
        " a.USER_statusAnketar, a.statusKontrolorNivo1, a.USER_statusKontrolorNivo1, " + nvrd +
        " (case when isnull(imastan,0)=0 then 0 else 1 end) as statusBoja" + nvrd +
    ...
    Sub refreshDGV2(Optional ByVal eRowIndex As Integer = -1, Optional datumzakazano As String = "", Optional status As String = "", Optional ByVal ulicakbroj As String = "", Optional ByVal novikbroj As String = "", Optional ByVal rbrstanZaDGV3 As String = "")
        ...
        If Not dt Is Nothing AndAlso dt.Rows.Count >= 0 Then
            ...
            DGV2.Columns("napomenaKontrolorNivo1").Visible = False
    ...
    Sub refreshDGV3(Optional ByVal eRowIndex As Integer = -1, Optional ByVal rbrstana As String = "", Optional ByVal ulica As String = "", Optional ByVal ime As String = "", Optional ByVal jmbg As String = "")
        ...
        If Not dt Is Nothing AndAlso dt.Rows.Count >= 0 Then
            ...
            DGV3.Columns("UStanuVanStana").Visible = True
            ...
            DGV3.Columns("UStanuVanStana").HeaderText = "Da li živi u stanu na današnji dan"
            ...
            DGV3.Columns("eeditnapomena").Width = 75
            DGV3.Columns("napomena").Width = 150
            DGV3.Columns("brojstana").Width = 100
            DGV3.Columns("UStanuVanStana").Width = 200
            ...
            If Not filterImePrezime Is Nothing Then
                ...
                filterImePrezime.Location = New Point(DGV3.RowHeadersWidth + DGV3.Columns("ulicakbroj").Width + DGV3.Columns("RbrStan").Width + DGV3.Columns("UStanuVanStana").Width + DGV3.Columns("rbrdom").Width + DGV3.Columns("rbrp1").Width + DGV3.Columns("brojstana").Width, DGV3.ColumnHeadersHeight - filterImePrezime.Height - 1)
    ...
    Function dajSQLzaDGV3Prazan() As String
        ...
        Dim str As String = "select  " + nvrd +
                            ...
                            " ,cast([ClanDom] as varchar(5)) as clandom,istnote, " + nvrd +
                            " [MBOPS], [Krug_idcd], [kucni_broj_idCD] " + nvrd +
                            " From spisaklica union all " + nvrd +
                            " Select [RbrStan],N'Ne živi u stanu na današnji dan' as UStanuVanStana,'/',rbrp1,Ime,Prezime,ImeRoditelja,pol,[Dan],[Mesec],[GodRod],rest,[Starost],'/','/',istnote, " + nvrd +
                            " [MBOPS], [Krug_idcd], [kucni_broj_idCD] From [dbo].[SpisakLicaOstali]  ) a  " + nvrd +
                            " left Join p2_dom b on a.[MBOPS]=b.[MBOPS] And a.[Krug_idcd]=b.[Krug_idcd] And a.[kucni_broj_idCD]=b.[kucni_broj_idCD] " + nvrd +
                            " And a.[RbrStan]=b.[RbrStan] And a.rbrdom=b.rbrdom " + nvrd +
                            " left join adresar as adr on  a.[MBOPS]=adr.[MBOPS] And a.[Krug_idcd]=adr.[Krug_idcd] And a.[kucni_broj_idCD]=adr.[kucni_broj_idCD] And a.[RbrStan]=adr.[RbrStan] " + nvrd +
                            " left join p2_stan as st on a.[MBOPS]=st.[MBOPS] And a.[Krug_idcd]=st.[Krug_idcd] And a.[kucni_broj_idCD]=st.[kucni_broj_idCD] And a.[RbrStan]=st.[RbrStan] " + nvrd +
    ...
    Sub srediGridView222()
        ...
        Me.filterStatus = New CCombobox2()
        ...
        filterStatus.Items.Add(New CComboboxItem2("     - popisivanje objekta nije započeto", Color.Black, Color.White))
        filterStatus.Items.Add(New CComboboxItem2("0   - popisivanje objekta/stanova u objektu u toku", Color.Black, Color.Yellow))
        filterStatus.Items.Add(New CComboboxItem2("2   - popisivanje objekta/stanova u objektu završeno", Color.Black, Color.LimeGreen))
        filterStatus.Items.Add(New CComboboxItem2("*   - svi", Color.Black, Color.White))
    ...
    Sub Reset()
        ...
        If nizACTB Is Nothing OrElse nizACTB.Length = 0 Then
            ...
            'strSQl = "select distinct left(krug_idcd,4) As displaymember, krug_idcd As valuemember from Adresar where IDANKETAR=N'" + kosam + "' order by left(krug_idcd,4)" 'DODATI WHERE ANKETAR = LOGOVAN ANKETAR
            strSQl = " select distinct [Krug_idcd] As valuemember, (case when exists (select left(krug_idcd,4) from adresarsvikrugovi where left(left(krug_idcd,4),4)=left(a.krug_idcd,4) and krug_idcd<>a.krug_idcd)" + nvrd +
                     " then krug_idcd else left(krug_idcd,4) end) As displaymember FROM [PES2021].[dbo].[AdresarSviKrugovi] a" + nvrd +
        ...
        If Not nizACTB Is Nothing AndAlso nizACTB.Length > 0 Then
            ...
            Else
                ...
                atbUlica.Text = ""
                'strSQl = "select distinct left(krug_idcd,4) as displaymember,krug_idcd as valuemember from Adresar " +
                ...
                strSQl = " select distinct [Krug_idcd] As valuemember, " + nvrd +
                                       ...
                                       " FROM [PES2021].[dbo].[AdresarSviKrugovi] a " + nvrd +
        ...
        Else
            Me.filterStatusDGV2 = New CCombobox2()
            ...
            filterStatusDGV2.Items.Add(New CComboboxItem2("     - popisivanje nije započeto", Color.Black, Color.White))
            filterStatusDGV2.Items.Add(New CComboboxItem2("0   - popisivanje u toku", Color.Black, Color.Yellow))
            filterStatusDGV2.Items.Add(New CComboboxItem2("1   - popisivanje završeno, podaci će sledećom sinhronizacijom biti poslati", Color.Black, Color.PaleGreen))
            filterStatusDGV2.Items.Add(New CComboboxItem2("2   - popisivanje završeno, podaci poslati na server", Color.Black, Color.LimeGreen))
            filterStatusDGV2.Items.Add(New CComboboxItem2("3   - Popisivač je zatražio od instruktora mogućnost ispravke poslatog stana ", Color.Black, Color.Gainsboro))
            filterStatusDGV2.Items.Add(New CComboboxItem2("4   - Instruktor je popisivaču vratio stan na doradu", Color.Black, Color.Red))
            filterStatusDGV2.Items.Add(New CComboboxItem2("*   - svi", Color.Black, Color.White))
    ...
    Sub actbV0()
        ...
        If actb0Staro <> atbPK.Text Then
            ...
            atbUlica.sselectedText = ""
            atbUlica.Text = ""
            Dim strSQl As String = " select distinct ulica as displaymember, idulice as valuemember from Adresar " +
        ...
        If atbOps.sselectedValue <> "" AndAlso atbOps.sselectedText <> "" Then
            ...
            ElseIf actb.Name.Trim.ToUpper = "ATBPK" AndAlso atbPK.sselectedValue <> "" AndAlso atbPK.sselectedText <> "" Then
                ...
                Else
                    ...
                    btnInformation.Visible = False
                End If
                Dim strSQlupit As String = " select distinct ulica as displaymember, idulice as valuemember from Adresar " +
                                           " where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd ='" + atbPK.sselectedValue + "'" +
                                           " union" +
                                           " select '(*)' as displaymember, '000000000000' as valuemember order by displaymember"
            ...
            ElseIf actb.Name.Trim.ToUpper = "ATBPK" AndAlso (atbPK.sselectedValue = "" OrElse atbPK.sselectedText = "") Then
                Dim strSQlupit2 As String = " select distinct ulica as displaymember, idulice as valuemember from Adresar " +
                                           " where 2=3"
    ...
    Private Sub ButtonSave2_Click(sender As Object, e As EventArgs) Handles ButtonSave2.Click
                    ...
                    Select Case opc2
                        Case "Yes"
                            ...
                            If ok2 = "ok" Then
                                ...
                                Call pZakazivanjeStanaNonVisible()
                            Else
                                ISTMessageBox.Show("Error - delete!")
                            End If
        ...
        If txtUkupanBrLica.Text.Trim <> "" Then
            Dim str As String = "select isnull(UBrLicaStan,0) from p2_stan s  inner join adresar a on a.mbops=s.mbops and a.krug_idcd=s.krug_idcd and a.kucni_broj_idcd=s.kucni_broj_idcd and a.rbrstan=s.rbrstan " + nvrd +
                                " where a.idanketar=N'" + kosam + "' and s.mbops='" + txtMBOPS.Text.Trim + "' and s.krug_idcd='" + txtKrug.Text.Trim + "' " + nvrd +
        ...
        If ok1 <> "ok" Then
            ISTMessageBox.Show(strSQL + nvrd + nvrd + "is not executed! ER45", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    ...
    Private Sub ButtonOK_Click(sender As Object, e As EventArgs) Handles ButtonDodajAdr.Click
        ...
        Dim strProvera As String = "select * from Adresar a " + vbCrLf +
                " where IDANKETAR=N'" + kosam + "' and a.mbops='" + atbOps.sselectedValue + "' and a.krug_idcd='" + atbPK.sselectedValue + "' and ltrim(rtrim(a.ulica))=N'" + tboxNovaUlica.Text.Trim + "' "
        ...
        Dim strsql2 As String = " select max(cast(left(kucni_broj_idcd,12) as bigint)) " + nvrd +
                                ...
                                " where IDANKETAR=N'" + kosam + "' and sourceof_insert='ANK' and substring(kucni_broj_idcd,1,8)=substring('" + fiktivnianketar + "',2,8) ) a "
        ...
        Dim strsql3 As String = "insert into Adresar (MBOPS,Krug_idcd," + vbCrLf + '"insert into Adresar (MBOPS,Krug_idcd,Krug,kucni_broj," + vbCrLf +
       ...
       " MBNAS,NazivOpstine,NazivNaselja,'" + idulice + "',N'" + novinazivul.Trim + "'," +
       " N'" + tboxKucniBR.Text.Trim + "'," + TextBoxLat.Text.Trim + "," + TextBoxLong.Text.Trim + ",SK,PkuSk," + vbCrLf +
       " N'" + novinazivul.Trim + " " + tboxKucniBR.Text.Trim + "','ANK',N'" + kosam + "', getdate() " + vbCrLf +
       " from AdresarSviKrugovi where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd='" + atbPK.sselectedValue.Trim + "'"
        ...
        Dim ok1 As String = izvrsiSQL(strsql3, Me.DBConnectionString)
        If ok1 <> "ok" Then
            ISTMessageBox.Show(strsql3 + nvrd + nvrd + "is not executed! ER59", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles ButtonUnosNovogKBR.Click
        ...
        If atbUlica.sselectedValue <> "000000000000" Then 'OrElse selectedCellCount > 0 Then
            ...
            tboxkbr.Focus()
        Else
            Dim strSQlupit As String = "select distinct ulica As displaymember, idulice as valuemember from Adresar " + vbCrLf +
    ...
    Private Sub ButtonSave_Click(sender As Object, e As EventArgs) Handles ButtonSave.Click
        ...
        If Label12.Text = "I" Then
            ...
            Dim strProvera As String = "select ltrim(rtrim(a.kbroj)) from Adresar a left join p2_objekat b on a.kucni_broj_idcd=b.kucni_broj_idcd and a.krug_idcd=b.krug_idcd " + vbCrLf +
                " where IDANKETAR=N'" + kosam + "' and a.mbops='" + atbOps.sselectedValue + "' and a.krug_idcd='" + atbPK.sselectedValue + "' and a.idulice='" + vred5.Trim + "' and a.kbroj=N'" + vred + "'"
            Dim dtProvera As DataTable = izvrsiSQLvratiDT(strProvera, Me.DBConnectionString, Nothing)
            If dtProvera.Rows.Count > 0 Then
            ...
            Else
                ...
                Dim str As String = "insert into Adresar (MBOPS,Krug_idcd," + vbCrLf +
               " kucni_broj_idCD, RbrStan, RbrUKrugu, KrugDeo, kucni_broj_Deo, IDAnketar, MBNAS, NazivOpstine," + vbCrLf +
               " NazivNaselja,IDUlice,Ulica,KBroj,Latituda,Longituda,SK,PkuSk," + vbCrLf +
               ...
               " N'" + tboxkbr.Text.Trim + "','" + tboxLatituda.Text.Trim + "','" + tboxLongituda.Text.Trim + "'," + vbCrLf +
               " SK,PkuSk,Ulica+' '+N'" + tboxkbr.Text.Trim + "'," + vbCrLf +
               ...
               " where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd='" + atbPK.sselectedValue.Trim + "' and idulice='" + vred5.Trim + "'"
                ...
                If ok = "ok" Then
                    ...
                    If ok2 = "ok" Then
                    Else
                        ISTMessageBox.Show(str2 + nvrd + nvrd + "is not executed! ER60", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
                Else
                    ISTMessageBox.Show(str + nvrd + nvrd + "is not executed! ER61", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
        ...
        ElseIf Label12.Text = "U" Then
            ...
            'Dim opstina As String = atbOps.sselectedValue.Trim
            'Dim fileReader1 As String = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\LongLat.txt")
            ...
            If pom1 <> vred Then
                Dim strProvera As String = "select ltrim(rtrim(a.kbroj)) " + vbCrLf +
            ...
            Else
                str = " update adresar set kbroj=N'" + tboxkbr.Text.Trim + "' , longituda='" + tboxLongituda.Text.Trim + "', latituda='" + tboxLatituda.Text.Trim + "', ulicakbroj=N'" + vred6 + "'+' '+N'" + tboxkbr.Text.Trim + "', date_update=getdate(), user_update=N'" + kosam + "'" + vbCrLf +
            ...
            Dim ok As String = izvrsiSQL(str, Me.DBConnectionString)
            If ok = "nok" Then
                ISTMessageBox.Show(str + nvrd + nvrd + "is not executed! ER62", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
            ...
            Else
                If btnMapaIzmenaKBR.Tag = "izmena" Then
                    If opstina.Trim <> "" Then
                        ...
                        '    fileReader = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBRSaLongLat.txt")
                        '    fileReader = fileReader + atbPK.sselectedText.Trim.Substring(0, 4) + "$" + textboxUlica.Text + "$" + stariKBR + "$" + staraLong.Trim + "$" + staraLat.Trim + ";"
                        '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBRSaLongLat.txt", "", False)
                        '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBRSaLongLat.txt", "" + fileReader.Trim + "", False)
                        'End If
                        If tboxLongituda.Text = staraLong AndAlso tboxLatituda.Text = staraLat AndAlso tboxkbr.Text <> stariKBR Then
                            ...
                            fileReader2 = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\UpdateKBR.txt")
                            ...
                            My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\UpdateKBR.txt", "", False)
                            My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\UpdateKBR.txt", "" + fileReader2.Trim + "", False)
                        End If
                    End If
                End If

                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\LongLat.txt", "", False)
            End If
        ...
        End If
        refreshDGV1(pomocnaUlica, pomocna, pomocnaUlicaMapa, pomocnaNapomenaKBR)
        Dim STRpom As String = "select count(*) from P2_dom where mbops='" + atbOps.sselectedValue.Trim + "' and krug_idcd='" + atbPK.sselectedValue.Trim + "' and isnull(izabranodom,0)=1"
        Dim dtpom As DataTable = izvrsiSQLvratiDT(STRpom, DBConnectionString, Me.jezik)
        Dim brIzabranaDOm As String = ""
        If dtpom.Rows.Count > 0 Then
            brIzabranaDOm = dtpom.Rows(0).Item(0).ToString
        End If
        Dim STRpom2 As String = "select count(*) from P2_dom where mbops='" + atbOps.sselectedValue.Trim + "' and krug_idcd='" + atbPK.sselectedValue.Trim + "' And isnull(izabranodom,0)<>0"
        Dim dtpom2 As DataTable = izvrsiSQLvratiDT(STRpom2, DBConnectionString, Me.jezik)
        Dim brUkupnoDom As String = ""
        If dtpom2.Rows.Count > 0 Then
            brUkupnoDom = dtpom2.Rows(0).Item(0).ToString
        End If


        lblBrojKBR.Text = "Ukupno kućnih brojeva: " + DGV1.Rows.Count.ToString + "    Ukupno popisanih domaćinstava\Ukupan broj domaćinstava izabranih za kontrolu kvaliteta: " + brUkupnoDom + "\" + brIzabranaDOm + ""
                    ...
                    Select Case opc2
                        Case "Yes"
                            ...
                            Else
                                ...
                                Dim str As String = " delete from Adresar" + vbCrLf +
                                    " where mbops='" + atbOps.sselectedValue + "' and krug_idcd='" + atbPK.sselectedValue + "'" + vbCrLf +
                                ...
                                If ok = "ok" Then
                                    ...
                                    'If opstina.Trim <> "" Then
                                    '    Dim fileReader As String = ""
                                    '    fileReader = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBR.txt")
                                    '    fileReader = fileReader + atbPK.sselectedText.Trim.Substring(0, 4) + "$" + pomulica + "$" + pomkbr + ";"
                                    '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBR.txt", "", False)
                                    '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBR.txt", "" + fileReader.Trim + "", False)
                                    'End If
                                    If postojiMapa() = False Then
                                        ISTMessageBox.Show("Nemate instaliranu mapu. Molimo vas pozovite instruktora kako biste mogli da nastavite sa radom!")
                                        Me.Cursor = Cursors.Default
                                    ...
                                    If DGV1.Rows.Count = 0 Then
                                        ...
                                        Call actbVOps()
                                        Call actbV0()
                                        Dim strSQl2 As String = " select distinct ulica as displaymember, idulice as valuemember from Adresar " +
                                                 " where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd ='" + atbPK.sselectedValue + "'" +
                                                 " union" +
                                                 " select '(*)' as displaymember, '000000000000' as valuemember order by displaymember"
    ...
    Private Sub Cancel()
        ...
        '    '    Dim fileReader As String = ""
        '    '    fileReader = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBRSaLongLat.txt")
        '    '    fileReader = fileReader + atbPK.sselectedText.Trim.Substring(0, 4) + "$" + textboxUlica.Text + "$" + tboxkbr.Text + "$" + pomlong.Trim + "$" + pomlat.Trim + ";"
        '    '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBRSaLongLat.txt", "", False)
        '    '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\brisanjeKBRSaLongLat.txt", "" + fileReader.Trim + "", False)
        '    'End If
        'End If
    ...
    Private Sub ButtonNastavi_Click(sender As Object, e As EventArgs) Handles ButtonNastavi.Click
        ...
        '    'Dim korisnik As String = dt.Rows(0).Item(0)
        '    'korisnik = korisnik.Trim.ToUpper.Replace("STATSRB\", "")
        '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\Resources\SifOps.txt", "", False)
        '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\Resources\SifOps.txt", CStr(CInt(opstina.Trim)).Trim, False, System.Text.Encoding.ASCII)
        '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina.Trim + "\pk.txt", "", False)
        '    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina.Trim + "\pk.txt", atbPK.sselectedText.Trim.Substring(0, 4), False)
        ...
        Else
            Dim opstina As String = atbOps.sselectedValue.Trim
            If opstina.Trim <> "" Then
                Dim dir As New IO.DirectoryInfo("C:\IST\QGIS\PES_21\" + opstina.Trim + "")
                If dir.Exists Then
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\Resources\SifOps.txt", "", False)
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\Resources\SifOps.txt", CStr(CInt(opstina.Trim)).Trim, False, System.Text.Encoding.ASCII)
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina.Trim + "\pk.txt", "", False)
    ...
    Private Function postojiMapa() As Boolean
        If atbOps.sselectedValue.Trim <> "" Then
            Dim dir As New IO.DirectoryInfo("C:\IST\QGIS\PES_21\" + atbOps.sselectedValue.Trim.Trim + "")
            If dir.Exists Then
                Return True
            Else
                Return False
    ...
    Private Sub ButtonZahtev_Click(sender As Object, e As EventArgs) Handles ButtonZahtev.Click
        ...
        If cbZahtev.Checked = False AndAlso tboxNapomenaStatusAnketar.Text.Trim <> "" Then
            ISTMessageBox.Show("Morate čekirati da želite da zatražite zahtev za doradu!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            cbZahtev.Focus()
            Exit Sub
        End If
        ...
        If vred2 = "3" AndAlso tboxNapomenaStatusAnketar.Text.Trim = "" Then
            ISTMessageBox.Show("Razlog ne sme biti prazan!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        ...
        If ok = "nok" Then
            ISTMessageBox.Show(strSQL + nvrd + nvrd + "is not executed! ER63", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    ...
    Private Sub ButtonNapomenaOStanuOK_Click(sender As Object, e As EventArgs) Handles btnOKNapomenaStan.Click
        ...
        Dim ok As String = izvrsiSQL(strSQL, Me.DBConnectionString)
        If ok = "nok" Then
            ISTMessageBox.Show(strSQL + nvrd + nvrd + "is not executed! ER64", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    ...
    Private Sub kucnibrojeviMapaBojenje()
        ...
        Dim FilePath = "C:\IST\QGIS\PES_21\" + opstina & "\" & FileName  'First Name of Directory and Then Name of Folder if it exists and then attach the name of file you want to search.

        If System.IO.File.Exists(FilePath) Then
            If opstina.Trim <> "" AndAlso krug.Trim <> "" Then
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IndikatorKBR.txt", "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IndikatorNKBR.txt", "", False)

                Dim str As String = " select distinct * from (select distinct cast(left(a.kucni_broj_idcd,12) as bigint) kucnibroj," + nvrd +
                                    ...
                                    " when isnull(b.adresateren,0)=4 Or isnull(vrstazgrade,0) in (5,6) Or (isnull(vrstazgrade,0)=4 And isnull(b.brojstanova,9999)=0) Or " + nvrd +
                                    " isnull(b.brojstanova, 9999) = (select count(*) from p2_stan a where a.mbops=b.mbops And a.kucni_broj_idCD=b.kucni_broj_idCD And a.KRUG_IDCD=b.KRUG_IDCD And a.qc=1)" + nvrd +
                                    " then '2' end) as Status, a.mbops, a.krug_idcd, a.sourceof_insert, a.ulica, a.kbroj, a.idanketar " + nvrd +
                ...
                Next
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IndikatorKBR.txt", "" + aa.Trim + "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IndikatorNKBR.txt", "" + bb.Trim + "", False)
            Else
                ISTMessageBox.Show("Morate izabrati opštinu i popisni krug.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
        End If
    ...
    Private Sub LinkLabel1_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles llOVDE.LinkClicked
        ...
        If dt.Rows.Count <= 0 Then
            ISTMessageBox.Show("Nemate unetih novih ulica", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            atbUlica.Focus()
            Exit Sub
        End If
        ...
        Dim strSQl As String = " select distinct ulica as displaymember, ulica as valuemember from Adresar " + vbCrLf +
    ...
    Private Sub ButtonIzmenaULOK_Click(sender As Object, e As EventArgs) Handles ButtonIzmenaULOK.Click
        ...
        Dim strSQlupit As String = " select distinct ulica as displaymember, idulice as valuemember from Adresar " + vbCrLf +
                                   " where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd ='" + atbPK.sselectedValue + "' and substring(IDULICE, 1, 8) ='" + Mid(fiktivnianketar, 2, 8) + "' and Ulica=N'" + atbPromenaNazivaUlice.sselectedText + "' " + vbCrLf +
                                   " order by displaymember"
        Dim dt2 As DataTable = izvrsiSQLvratiDT(strSQlupit, DBConnectionString, Me.jezik)
        If dt2.Rows.Count <= 0 Then
            ISTMessageBox.Show("Nije ispravan Stari naziv ulice", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            atbPromenaNazivaUlice.Focus()
            Exit Sub
        End If


        Dim strSQl As String = " select distinct ulica from Adresar" + vbCrLf +
                               " where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd='" + atbPK.sselectedValue + "' and ulica =N'" + txtNNUlice.Text.Trim + "'" + vbCrLf +
                               " order by ulica"
        ...
        If atbPromenaNazivaUlice.Text.Trim = "" Then
            ISTMessageBox.Show("Morate odabrati ulicu čiji naziv želite da promenite!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            atbPromenaNazivaUlice.Focus()
            Exit Sub
        ElseIf txtNNUlice.Text.Trim = "" Then
            ISTMessageBox.Show("Morate uneti naziv ulice!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            txtNNUlice.Focus()
            Exit Sub
        ElseIf dt.Rows.Count > 0 Then
            ISTMessageBox.Show("Morate uneti naziv ulice koji ne postoji!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
            txtNNUlice.Focus()
            Exit Sub
        Else
            vred5 = atbPromenaNazivaUlice.sselectedValue
            Dim str As String = " update adresar " +
                                " set ulica=N'" + txtNNUlice.Text + "',ulicakbroj=N'" + txtNNUlice.Text + "'+' '+kbroj,date_update=getdate(),user_update=N'" + kosam + "'" +
            ...
            If ok = "ok" Then
                ...
                atbUlica.sselectedText = ""
                atbUlica.Text = ""
                strSQl = " select distinct ulica as displaymember, idulice as valuemember from Adresar " +
                           " where IDANKETAR=N'" + kosam + "' and mbops='" + atbOps.sselectedValue + "' and krug_idcd ='" + atbPK.sselectedValue + "'" +
                           " union" +
                           " select '(*)' as displaymember, '000000000000' as valuemember order by displaymember"
                Call popuniAutbDB(atbUlica, strSQl)
            Else
                ISTMessageBox.Show(str + nvrd + nvrd + "is not executed! ER65", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            ...
            If postojiMapa() = False Then
                ISTMessageBox.Show("Nemate instaliranu mapu. Molimo vas pozovite instruktora kako biste mogli da nastavite sa radom!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Me.Cursor = Cursors.Default
            Else
                Dim opstina As String = atbOps.sselectedValue.Trim
                If opstina.Trim <> "" Then
                    Dim fileReader As String = ""
                    fileReader = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\IzmenaUL.txt")
                    fileReader = fileReader + atbPromenaNazivaUlice.sselectedText + "$" + txtNNUlice.Text.Trim + ";"
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IzmenaUL.txt", "", False)
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IzmenaUL.txt", "" + fileReader.Trim + "", False)
                End If
            End If
    ...
    Private Sub BtnOkNapomenaLica_Click(sender As Object, e As EventArgs) Handles BtnOkNapomenaLica.Click
        ...
        Dim ok As String = izvrsiSQL(strSQL, Me.DBConnectionString)
        If ok = "nok" Then
            ISTMessageBox.Show(strSQL + nvrd + nvrd + "is not executed! ER66", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    ...
    Private Sub BtnNUlica_Click(sender As Object, e As EventArgs) Handles ButtonUnosNoveUlice.Click
        ...
        Dim strSQl As String = "select distinct ulica As displaymember, ulica as valuemember from Adresar where mbops='" + atbOps.sselectedValue + "' and krug_idcd='" + atbPK.sselectedValue + "' order by ulica"
    ...
    Sub refreshdgv2zakazni(Optional ByVal datumzakazano As String = "", Optional ByVal status As String = "", Optional ByVal ulicakbroj As String = "", Optional ByVal novikbroj As String = "")
        ...
        " And exists (select * from P2_stan x where a.mbops=x.mbops And a.krug_idcd=x.krug_idcd And a.kucni_broj_idcd=x.kucni_broj_idcd And a.rbrstan=x.rbrstan ) and cast(isnull(a.statusAnketar,'') as varchar(1)) in ('','0','1')" + nvrd +
        " then N'🗑' else '' end) as DeleteIZStana," + nvrd +
        " a.kucni_broj_Deo,a.Krug_idcd, a.idulice,a.kucni_broj_idCD,a.ulica,a.kbroj,a.cati_nacin, " + nvrd +
        " a.mbops, a.NazivOpstine, a.mbnas, " + nvrd +
        " a.sourceof_INSERT, a.ishod, " + nvrd +
        " a.USER_statusAnketar, a.statusKontrolorNivo1, a.USER_statusKontrolorNivo1, " + nvrd +
        " (case when isnull(imastan,0)=0 then 0 else 1 end) as statusBoja" + nvrd +
        ...
        " where isnull(ZakazanoDatum,'')<>'' and isnull(ZakazanoDatum,' ') like '%" + datumzakazano + "%' and isnull(statusanketar,' ') like N'%" + status + "%' and isnull(ulicakbroj,' ') like N'%" + ulicakbroj + "%' and isnull(NoviKbrojTeren,' ') like N'%" + novikbroj + "%'  order by a.ZakazanoDatum"
    ...
    Sub refreshdgv3svalica(Optional ByVal fulica As String = "", Optional ByVal fime As String = "", Optional ByVal fjmbg As String = "")
        ...
        Dim str As String = "select  " + nvrd +
                            " a.[RbrStan],ulicakbroj,brojstana" + nvrd +
                            " ,a.rbrdom,a.rbrp1" + nvrd +
                            " ,ime+' ' +prezime as ImePrezime" + nvrd +
                             " ,[ImeRoditelja],isnull([Pol],'') as pol " + nvrd +
                            " ,case when rest='999999' then [Dan]+[Mesec]+[GodRod]+rest else [Dan]+[Mesec]+[GodRod]+substring('*******************************************************',1,len(ltrim(rtrim(rest)))) end  as jmbg " + nvrd +
                            ...
                            " ,isnull(a.istnote,'') as napomena, ' ' as praznoY " + nvrd +
                            " ,a.[MBOPS],a.[Krug_idcd] " + nvrd +
                            " ,a.[kucni_broj_idCD],idulice,adr.sourceof_INSERT ,  ime + ' ' + prezime as ImePrezimeCelo" + nvrd + '" ,a.ssrodstvo " + nvrd +
                            " ,(case when isnull(adr.statusAnketar,0)=0 and isnull(adr.USER_statusAnketar,'') not like 'I7%' and  isnull(adr.USER_statusAnketar,'') not like 'I8%' then '0' " + nvrd +
                            " when isnull(adr.statusAnketar,0)=0 and (isnull(adr.USER_statusAnketar,'') like 'I7%' or isnull(adr.USER_statusAnketar,'') like 'I8%') then '4' " + nvrd +
                            " else cast(isnull(adr.statusAnketar,'') as varchar(1)) end) as statusAnketar" + nvrd +
                            ...
                            " left join adresar as adr on  a.[MBOPS]=adr.[MBOPS] and a.[Krug_idcd]=adr.[Krug_idcd] and a.[kucni_broj_idCD]=adr.[kucni_broj_idCD] and a.[RbrStan]=adr.[RbrStan] " + nvrd +
                            " left join p2_stan as st on a.[MBOPS]=st.[MBOPS] and a.[Krug_idcd]=st.[Krug_idcd] and a.[kucni_broj_idCD]=st.[kucni_broj_idCD] and a.[RbrStan]=st.[RbrStan] " + nvrd +
                            " where IDANKETAR=N'" + kosam + "' and ulicakbroj like N'%" + fulica + "%' and (ime + ' ' + prezime like N'%" + fime + "%') and dan+mesec+godrod+rest like '%" + fjmbg + "%'"
    ...
    Private Sub btnMapa_Click(sender As Object, e As EventArgs) Handles btnMapa.Click, btnMapaKbr.Click
        ...
        Try
            ...
            Dim prepisinove As String = "select distinct left(krug_idcd,4), longituda, latituda, Ulica, kbroj, PKuSK, SK from adresar where sourceof_INSERT='ANK' and isnull(naslediood,'')<>'' and IDAnketar=N'" + kosam + "' and isnull(KBROdDrugogDodatNaMapu,0)<>1"
            ...
            Dim opstina As String = atbOps.sselectedValue.Trim
            My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\PrepisiTacke.txt", "", False)
            My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\PrepisiTacke.txt", strzatxt, False)



            Dim sreditacke As String = "select distinct left(krug_idcd,4), longituda, latituda, Ulica, kbroj from adresar where sourceof_INSERT='ANK' and IDAnketar=N'" + kosam + "'"
            ...
            If dtsreditacke.Rows.Count > 0 Then
                ...
                Next
            End If
            My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\SrediTacke.txt", "", False)
            My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\SrediTacke.txt", sreditacketxt, False)
            ...
            If opstina.Trim <> "" Then
                'Dim korisnik As String = dt.Rows(0).Item(0)
                'korisnik = korisnik.Trim.ToUpper.Replace("STATSRB\", "")
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\formaMape.txt", "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\formaMape.txt", kbr.Trim, False)
                '''''TACKANJE PREMA ULICIIII
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IndikatorUlica.txt", "", False)
                ...
                ElseIf kbr = "" Then
                    If atbUlica.sselectedText <> "" Then
                        ...
                        My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\IndikatorUlica.txt", prepisiul, False)
                    End If
                End If
                ...
                If kbr = "KBR" Then
                    ...
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\ulica.txt", "", False)
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\ulica.txt", "'" + ulica.Trim + "'", False)

                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\kbr.txt", "", False)
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\kbr.txt", "'" + tboxkbr.Text.Trim + "'", False)

                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\krug.txt", "", False)
                    My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\krug.txt", "'" + atbPK.sselectedText.Trim.Substring(0, 4) + "'", False)
            ...
            Me.Cursor = Cursors.WaitCursor
            Dim myProcess As Process = Process.Start("C:\IST\QGIS\PES_21\Resources\qgis_v2.bat")
            ...
            psi.WorkingDirectory = "C:\IST\QGIS\PES_21\" + atbOps.sselectedValue.Trim + "\"
            psi.WindowStyle = System.Diagnostics.ProcessWindowStyle.Maximized
            Dim ppp As System.Diagnostics.Process = System.Diagnostics.Process.Start(psi)
            ppp.WaitForExit()
            overlayHandle.Close()
            ISTMessageBox.Show("Rad sa mapom je završen, možete nastaviti sa radom u aplikaciji za unos.", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)


            Me.Cursor = Cursors.Default


            Dim fileReader11 As String = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\PrepisiTacke.txt")
            If fileReader11.Trim = "" Then
                Dim ok1 As String = izvrsiSQL(updateprepisane, Me.DBConnectionString)
                If ok1 <> "ok" Then
                    ISTMessageBox.Show(updateprepisane + nvrd + nvrd + "is not executed! ER46", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub btnMapaIzmena_Click(sender As Object, e As EventArgs) Handles btnMapaIzmenaKBR.Click
        ...
        Try
            ...
            Dim strProvera As String = "select distinct ltrim(rtrim(a.kbroj)) from Adresar a left join p2_objekat b on a.kucni_broj_idcd=b.kucni_broj_idcd And a.krug_idcd=b.krug_idcd " + vbCrLf +
                       " where IDANKETAR=N'" + kosam + "' and a.mbops='" + atbOps.sselectedValue + "' and a.krug_idcd='" + atbPK.sselectedValue + "' and a.idulice='" + textboxIDUlice.Text.Trim + "' and a.kbroj=N'" + vred + "'"
            Dim dtProvera As DataTable = izvrsiSQLvratiDT(strProvera, Me.DBConnectionString, Nothing)
            ...
            If opstina.Trim <> "" Then
                ...
                'korisnik = korisnik.Trim.ToUpper.Replace("STATSRB\", "")
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\formaMape.txt", "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\formaMape.txt", "KBR", False)

                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\ulica.txt", "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\ulica.txt", "'" + textboxUlica.Text + "'", False)
            ...
            '    Dim info As New ProcessStartInfo("C:\IST\QGIS\PES_21\Resources\qgis_v2.bat")
            ...
            If fileReader11.Trim = "" Then
                Dim ok1 As String = izvrsiSQL(updateprepisane, Me.DBConnectionString)
                If ok1 <> "ok" Then
                    ISTMessageBox.Show(updateprepisane + nvrd + nvrd + "is not executed! ER47", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub llNastavi_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles llNastavi.LinkClicked
        ...
        'Dim opstina As String = atbOps.sselectedValue.Trim
        'If opstina.Trim <> "" Then
        '    Dim fileReader As String = My.Computer.FileSystem.ReadAllText("C:\IST\QGIS\PES_21\" + opstina + "\LongLat.txt")
    ...
    Private Sub btnMapaKbrUL_Click(sender As Object, e As EventArgs) Handles btnMapaKbrUL.Click
        ...
        Try
            ...
            If opstina.Trim <> "" Then
                ...
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\ulica.txt", "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\ulica.txt", "'" + tboxNovaUlica.Text.Trim + "'", False)

                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\kbr.txt", "", False)
                My.Computer.FileSystem.WriteAllText("C:\IST\QGIS\PES_21\" + opstina + "\kbr.txt", "'" + tboxKucniBR.Text.Trim + "'", False)
            ...
            If fileReader11.Trim = "" Then
                Dim ok1 As String = izvrsiSQL(updateprepisane, Me.DBConnectionString)
                If ok1 <> "ok" Then
                    ISTMessageBox.Show(updateprepisane + nvrd + nvrd + "is not executed! ER48", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information)
                End If
            End If
    ...
    Private Sub btnVracanjeKrugova_Click(sender As Object, e As EventArgs) Handles btnVracanjeKrugova.Click
        ...
        'Dim strOps As String = " Select  distinct aa.mbops As valuemember, nazivopstine As displaymember  " + nvrd +
        '                    " From [PES2021].[dbo].[AdresarSviKrugovi] aa  " + nvrd +
        '                    " inner Join(select distinct mbops, nazivopstine from adresar where IDANKETAR=N'" + kosam + "') b on aa.mbops=b.mbops   " + nvrd +
        '                    " where IDANKETAR = N'" + kosam + "' and isnull([IndZavrsenPop],0)<>1 and isnull([IndPrazan],0)<>1   "

        'Dim dtOps As DataTable = izvrsiSQLvratiDT(strOps, Me.DBConnectionString, Me.jezik)

        'If dtOps.Rows.Count > 0 Then
        '    If dtOps.Rows.Count > 1 Then
        '        Call popunicbo(cmbVratiPKUOps, strOps, DBConnectionString, True)
        '        cmbVratiPKUOps.Enabled = True

        '    Else
        '        Call popunicbo(cmbVratiPKUOps, strOps, DBConnectionString, False)
        '        cmbVratiPKUOps.Enabled = False
        '    End If
        '    PanelPKDorada.Visible = True
        'Else
        '    ISTMessageBox.Show("Nemate krugove koje možete da tražite na doradu. U pitanju je greška!")
        '    PanelPKDorada.Visible = False
        ...
        Dim str2 As String = " select distinct [Krug_idcd]+'$'+a.mbops As valuemember, " + nvrd +
                             ...
                             " From [PES2021].[dbo].[AdresarSviKrugovi] a " + nvrd + 'inner Join(Select distinct mbops, nazivopstine from adresar where IDANKETAR= N'" + kosam + "') b on a.mbops=b.mbops  
    ...
    'Private Sub labelsToolTip_Popup(sender As Object, e As PopupEventArgs) Handles labelsToolTip.Popup
    '    Me.labelsToolTip.Show(Text, lblInformation, lblInformation.PointToClient(Cursor.Position), 60000)
    'End Sub



    'Private Sub txtUkupanBrLica_Validation(sender As Object, e As EventArgs) Handles txtUkupanBrLica.Validating
    ...
    '                            " where a.idanketar=N'" + kosam + "' and s.mbops='" + atbOps.sselectedValue.Trim + "' and s.krug_idcd='" + atbPK.sselectedValue.Trim + "' " + nvrd +
    '                            " and s.kucni_broj_idcd='" + txtkucni_broj_idCD.Text.Trim + "' and s.rbrstan=" + txtRbrStan.Text.Trim + ""
    '    Dim dt As DataTable = izvrsiSQLvratiDT(str, DBConnectionString, Me.jezik)
    ...
    '    If Not TabPage2.Tag Is Nothing Then
    '        If TabPage2.Tag = "bezstanova" Then
    '            ISTMessageBox.Show("Nema unetih stanova u okviru ovog kućnog broja.")
    '        ElseIf TabPage2.Tag = "nijeodabrano" Then
    '            ISTMessageBox.Show("Odaberite kućni broj kako biste mogli da pristupite pregledu stanova.")
    '        End If
    '    End If
    'End Sub
    ...
    '    If Not TabPage3.Tag Is Nothing AndAlso TabPage3.Tag = "bezlica" Then
    '        ISTMessageBox.Show("Nema unetih lica u okviru ovog stana.")
    '    End If
...
'    End Sub
'End Module
Class CCombobox2 : Inherits ComboBox
    Sub New()
    ...
    Protected Overrides Sub OnDrawItem(e As DrawItemEventArgs)
        MyBase.OnDrawItem(e)
        Dim item As CComboboxItem2 = DirectCast(Me.Items(e.Index), CComboboxItem2)
