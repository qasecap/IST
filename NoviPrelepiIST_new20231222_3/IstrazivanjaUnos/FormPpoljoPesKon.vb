...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(FormPpoljoPesKon))
        ...
        Me.PanelLegendaDGV1.Size = New System.Drawing.Size(711, 130)
        ...
        Me.Label3.Location = New System.Drawing.Point(313, 105)
    ...
    "___________________________________"
        ...
        Me.gbStatusi.Location = New System.Drawing.Point(831, 217)
        Me.gbStatusi.Name = "gbStatusi"
        Me.gbStatusi.Size = New System.Drawing.Size(356, 41)
        ...
        Me.rbSve.Location = New System.Drawing.Point(299, 18)
        Me.rbSve.Name = "rbSve"
        ...
        Me.rbVraceno.Location = New System.Drawing.Point(159, 18)
...
#End Region
    ...
    Public Sub New(ByVal dist As DatasetIstrazivanja, ByVal ttab As DatasetIstrazivanja.ISTTABSDataTable, ByVal ri As DatasetIstrazivanja.ISTRow,
        ...
        If rbZaPlacanje.Checked = True Then
    ...
    Private Sub FormDnevnikAnketiranja_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        gbStatusi.Location = New Point(PanelLegendaDGV1.Location.X, PanelLegendaDGV1.Location.Y + PanelLegendaDGV1.Height + 30)
        gbFilteri.Location = New Point(PanelLegendaDGV1.Location.X + gbStatusi.Width + 10, gbStatusi.Location.Y)
    ...
    Sub Dgv1CellContentClick(ByVal erowindex As Integer, ByVal eColumnIndex As Integer)
        ...
        If erowindex >= 0 Then
            ...
            ElseIf DGV1.Columns(eColumnIndex).DisplayIndex = DGV1.Columns("PoglPodaciKPP").DisplayIndex AndAlso DGV1.Rows(erowindex).Cells(DGV1.Columns("PoglPodaciKPP").Index).Value.ToString <> "x" Then

                gbSecurity.Visible = True
                Dim zzak As Boolean = False
                Dim red As Integer = erowindex
                Dim tabela As String = "prijemPodaci_KPP"
    ...
    Sub ZahtevZaPlacanjeAkcija(ByVal selectedRow As Integer)
        ...
        If ZahtevZaPlacanje.Trim.ToUpper = "FALSE" AndAlso StatusKontrolorNivo1.Trim.ToUpper.Substring(0, 1) = "0" Then
            If StatusAnketar.Trim.ToUpper = "2" Then
                sql = " update adresar set StatusKontrolorNivo1=6, User_StatusKontrolorNivo1=N'" + kosam + "', DATE_statusKontrolorNivo1=getdate()" + nvrd +
        ...
        ElseIf ZahtevZaPlacanje.Trim.ToUpper = "TRUE" AndAlso StatusKontrolorNivo1.Trim.ToUpper.Substring(0, 1) = "6" Then
            sql = " update adresar set StatusKontrolorNivo1=0, User_StatusKontrolorNivo1=N'" + kosam + "', DATE_statusKontrolorNivo1=getdate()" + nvrd +
                  " where pig='" + PIG + "' and rbr='" + RBR + "' "
        End If
    ...
    Sub refreshDGV1(ByVal filterNas As String, ByVal filtPigg As String, ByVal filtPopisivacc As String, Optional ByVal filterstatus As String = "")
        ...
        str = "drop table if exists #aa; select * into #aa from imtrep3.dbo.vPP23_VezaOKPOP_IBbroj_OpsKoor where isnull(odustao,0)=1 and sifraok='" + kosam + "' ;" + nvrd +
              " drop table if exists #bb; select * into #bb from Veza_Ank_Kont_Po where opstinskikoor=N'" + kosam + "' ;" + nvrd +
              " Select (case when left(isnull(statuskontrolornivo1,0),1) In (6,7) Then cast(1 As bit) When isnull(statusKontrolorNivo1,0)=0 Then cast(0 As bit) Else cast(0 As bit) End) As ZahtevZaPlacanje," + nvrd +
              " a.anketar,a.PIG, a.rbr, (case when ( isnull(cast(P1.indgr as int),0)>0) then 'Da' else 'Ne' end) as GR, " + nvrd +
              " N'' as Prazno0,  " + nvrd +
              " (case when isnull(a.statusAnketar,0) in (2,3) and left(isnull(statuskontrolornivo1,0),1) not in (6,7) then N'📝' else '' end ) PoglPodaciKPP, N'' as PraznoB, " + nvrd +
              " PravniStatus, " + nvrd +
              " (case when isnull(trim(case when trim(ime)='' then NULL else ime end),'')<>'' then " + nvrd +
              " trim(case when trim(ime)='' then NULL else ime end)+' '+trim(case when trim(Prezime)='' then NULL else Prezime end) " + nvrd +
              " when isnull(trim(case when trim(NazivPL)='' then NULL else NazivPL end),'')<>'' then trim(case when trim(NazivPL)='' then NULL else NazivPL end) else '' end) as ImePrezime, " + nvrd +
              " Nazivnaselja, NASELJe,  " + nvrd +
              " trim(case when trim(ulica)='' then NULL else ulica end)+' '+ trim(case when trim(kbr)='' then NULL else kbr end) Adresa, " + nvrd +
              " OPstina, Nazivopstine," + nvrd +
              " N'' as Prazno1, N'🖉' as Napomena, ISTNote,  N'' as PraznoD, " + nvrd +
              " (case when isnull(a.webunos,0)=0 and isnull(a.statusAnketar,0)=3 and left(isnull(statuskontrolornivo1,0),1)<>7 and not exists (select * from #aa x where  a.anketar=x.sifra)" + nvrd +
              " then N'🖐' when isnull(a.webunos,0)=0 and  isnull(a.statusAnketar,0)=2 and left(isnull(statuskontrolornivo1,0),1)<>7 and not exists (select * from #aa x where a.anketar=x.sifra) then N'✉' else '' end ) as ZahtevDorada, a.napomenaKontrolorNivo1 ,  a.napomenaStatusAnketar RazlogZahteva, " + nvrd +
              " (case when isnull(a.statusAnketar,0)=2 and left(trim(isnull(a.user_statusAnketar,'')),1) = 'P' and isnull(a.capiexported,0)=1 then 'Da' else '' end ) as Ispravljeno , N'' as PraznoA, " + nvrd +
              " god, (case when left(isnull(statuskontrolornivo1,0),1)=7 then 'PLACENI'" + nvrd +
              ...
              " from (select * from adresar xx where exists (select * from #bb b where xx.anketar=b.anketar ) and " + nvrd + 'aktivan=1
              " (isnull(statusanketar,0) in (2,3) or (isnull(statusanketar,0)=0 and left(isnull(trim(user_statusanketar),'9999'),2) in ('I7','I8')))) a " + nvrd +
              " left join (select pig, rbr, indgr, odziv, p080,p411,p199p,p215p,p221p,p226p,p237p,p240p " + nvrd +
              " from PrijemPodaci_KPP) P1 on a.pig=P1.pig And a.rbr=P1.rbr " + nvrd +
              " where (isnull(p080,0)>=0.5 Or isnull(p411,0)=1 Or isnull(p199p,0)>=2 Or  (isnull(p199p,0)=1 And (isnull(p215p,0)>=2  Or isnull(p221p,0)>=2 Or " + nvrd +
              " isnull(p226p,0)>=2 Or (isnull(p215p,0)=1 And  isnull(p221p,0)>=1) Or (isnull(p215p,0)=1 And isnull(p226p,0)>=1) Or " + nvrd +
              " (isnull(p221p,0)=1 And isnull(p226p,0)>=1)) Or (isnull(p221p,0)>=5) Or (isnull(p226p,0)>=5) Or (isnull(p215p,0)>=3) Or " + nvrd +
              " (isnull(p215p,0)+isnull(p221p,0)+isnull(p226p,0)>=4) Or (isnull(p237p,0)>=50) Or (isnull(p240p,0)>=50)))" + nvrd +
              " And (isnull(cast(P1.Odziv As nvarchar(2)),'') in ('1', '7A') or (isnull(cast(P1.Odziv as nvarchar(2)),'')='' and isnull(statusanketar,0)=0 and left(isnull(trim(user_statusanketar),'9999'),2) in ('I7','I8'))) and Nazivnaselja like '%" + filterNas + "%' and a.pig like N'%" + filtPigg + "%' " + nvrd +
        ...
        ElseIf filterstatus.Trim.ToUpper = "PLACENI" Then
            str = str + nvrd + " and left(isnull(statuskontrolornivo1,0),1)=7 "
        ...
        'If checkbIsplaceniOnOFF.Checked = True Then
        '    str = str + " and isnull(statuskontrolornivo1,0) not in (71,72,73) "
        'End If
        ...
        DGV1.Columns("PoglPodaciKPP").HeaderText = "P1"
        ...
        DGV1.Columns("napomenaKontrolorNivo1").HeaderText = "Razlog vraćanja na doradu /" + nvrd + " Odbijanja zahteva za doradu"
        ...
        DGV1.Columns("PoglPodaciKPP").Width = 30
        ...
        DGV1.Columns("napomenaKontrolorNivo1").Width = 205
    ...
    Private Sub DGV1_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGV1.CellContentClick
        ...
        If e.ColumnIndex <> DGV1.Columns("Napomena").DisplayIndex AndAlso e.ColumnIndex <> DGV1.Columns("PoglPodaciKPP").DisplayIndex AndAlso e.ColumnIndex <> DGV1.Columns("ZahtevZaPlacanje").DisplayIndex AndAlso e.ColumnIndex <> DGV1.Columns("ZahtevDorada").DisplayIndex Then
            Exit Sub
        End If
    ...
    Private Sub rbPlaceni_CheckedChanged(sender As Object, e As EventArgs)
    ...
    Private Sub btnZahtevOK_Click(sender As Object, e As EventArgs) Handles btnZahtevOK.Click
        ...
        If DGV1.Rows(kojired).Cells(DGV1.Columns("ZahtevDorada").Index).Value.ToString.Trim = "🖐" Then
                ...
                If status = "ZAPLACANJE" Then
                    ...
                    Select Case opc
                        Case "Da"
                            ...
                            Select Case opc2
                                Case "Da"
                                    Dim sql1 As String = "update adresar set statusanketar=0, user_statusanketar=N'" + kosam + "', DATE_statusAnketar=getdate(), capiexported=1, statuskontrolornivo1=0 where pig='" + tboxZahtevPig.Text.Trim + "' and rbr=" + tboxZahtevRbr.Text.Trim + ""
                    ...
                    End Select
                Else
                    Dim sql1 As String = "update adresar set statusanketar=0, user_statusanketar=N'" + kosam + "', DATE_statusAnketar=getdate(), capiexported=1 where pig='" + tboxZahtevPig.Text.Trim + "' and rbr=" + tboxZahtevRbr.Text.Trim + ""
                    Dim ok1 As String = izvrsiSQL(sql1, DBConnectionString)
                    If ok1 <> "ok" Then
            ...
            If rbZahtevOdbijen.Checked = True Then
                ...
                Dim sql1 As String = "update adresar set statusanketar=2, user_statusanketar=N'" + kosam + "', DATE_statusAnketar=getdate(), napomenaKontrolorNivo1=N'" + tboxRazlogZahtev.Text + "'  where pig='" + tboxZahtevPig.Text.Trim + "' and rbr=" + tboxZahtevRbr.Text.Trim + ""
                Dim ok1 As String = izvrsiSQL(sql1, DBConnectionString)
                If ok1 <> "ok" Then
                ...
                If status = "ZAPLACANJE" Then
                    ...
                    Select Case opc
                        Case "Da"
                            ...
                            Select Case opc2
                                Case "Da"
                                    Dim sql1 As String = "update adresar set statusanketar=0, user_statusanketar=N'" + kosam + "', DATE_statusAnketar=getdate(), capiexported=1, statuskontrolornivo1=0, napomenaKontrolorNivo1=N'" + tbRazlogVracanja.Text + "'  where pig='" + tboxZahtevPig.Text.Trim + "' and rbr=" + tboxZahtevRbr.Text.Trim + ""
                                    Dim ok1 As String = izvrsiSQL(sql1, DBConnectionString)
                                    If ok1 <> "ok" Then
                    ...
                    End Select
                Else
                    Dim sql1 As String = "update adresar set statusanketar=0, user_statusanketar=N'" + kosam + "', DATE_statusAnketar=getdate(), capiexported=1, napomenaKontrolorNivo1=N'" + tbRazlogVracanja.Text + "'  where pig='" + tboxZahtevPig.Text.Trim + "' and rbr=" + tboxZahtevRbr.Text.Trim + ""
                    Dim ok1 As String = izvrsiSQL(sql1, DBConnectionString)
                    If ok1 <> "ok" Then
    ...
    Private Sub btnNapomenaOK_Click(sender As Object, e As EventArgs) Handles btnNapomenaOK.Click
        ...
        If tboxNapomenaNapomena.Text.Trim = "" Then
            ...
            Exit Sub
        Else
            Dim sql1 As String = "update adresar set istnote=N'" + tboxNapomenaNapomena.Text + "' , ko_UPDATE=N'" + kosam + "' , datum_UPDATE = getdate() where pig='" + tboxNapomenaPIG.Text.Trim + "' and rbr=" + tboxNapomenaRBR.Text.Trim + ""
            Dim ok1 As String = izvrsiSQL(sql1, DBConnectionString)
            If ok1 <> "ok" Then
