...
Public Class mFormExitEventColor
...
#End Region
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.Label1.Text = "BackColor for field " + pp(2).Trim + "    Type: " + CStr(pp(5)).Trim + ", MaxLength= " + CStr(pp(6)).Trim
    End Sub
    ...
    Sub getMetaDataPane()
        ...
        Dim colorif() As String = {}
        Dim colorto() As String = {}
        Dim colorwhat() As String = {}

        For i As Integer = 0 To pp.GetUpperBound(0)
            If pp(i).Length > 7 AndAlso pp(i).Trim.ToUpper.IndexOf("COLORIF") >= 0 OrElse pp(i).Length > 6 AndAlso pp(i).Trim.ToUpper.IndexOf("BOJIAKO") >= 0 Then
                colorif = pp(i).Trim.ToUpper.Replace("COLORIF=", "").Replace("BOJIAKO=", "").Split("$")
            ElseIf pp(i).Length > 7 AndAlso pp(i).Trim.ToUpper.IndexOf("COLORTO") >= 0 OrElse pp(i).Length > 4 AndAlso pp(i).Trim.ToUpper.IndexOf("BOJIU") >= 0 Then
                colorto = pp(i).Trim.ToUpper.Replace("COLORTO=", "").Replace("BOJIU=", "").Split("$")
            ElseIf pp(i).Length > 9 AndAlso pp(i).Trim.ToUpper.IndexOf("COLORWHAT") >= 0 OrElse pp(i).Length > 6 AndAlso pp(i).Trim.ToUpper.IndexOf("BOJISTA") >= 0 Then
                colorwhat = pp(i).Trim.ToUpper.Replace("COLORWHAT=", "").Replace("BOJISTA=", "").Split("$")
            ElseIf pp(i).Trim <> "" Then
                validatingEventOst = validatingEventOst + pp(i) + ";"
            End If
        Next

        For i As Integer = 0 To colorif.GetUpperBound(0)
        ...
        For i As Integer = 0 To colorif.GetUpperBound(0)
            ...
            If Not tb Is Nothing Then DirectCast(tb, TextBox).Text = colorif(i).Trim
        Next

        For i As Integer = 0 To colorto.GetUpperBound(0)
            Dim tb As Control = NadjiKontrolu(Me, "tb" + CStr(i + 1).Trim)
            If Not tb Is Nothing Then DirectCast(tb, TextBox).Text = colorto(i).Trim
        Next
        For i As Integer = 0 To colorwhat.GetUpperBound(0)
            Dim tb As Control = NadjiKontrolu(Me, "tc" + CStr(i + 1).Trim)
            If Not tb Is Nothing Then DirectCast(tb, TextBox).Text = colorwhat(i).Trim
        Next
    ...
    Private Sub btn_Click(sender As Object, e As EventArgs) Handles btn.Click
        ...
        For k As Integer = 1 To 20
            ...
            If Not tb Is Nothing AndAlso DirectCast(tb, TextBox).Text <> "" Then
                colorif = colorif + tb.Text + "$"
                br = br + 1
            End If
        Next
        If colorif <> "" Then
            colorif = "colorif=" + colorif.TrimEnd("$") + ";"
            kkon = kkon + colorif
        End If
        ...
        For k As Integer = 1 To br
            ...
            If DirectCast(tb, TextBox).Text <> "" Then colorto = colorto + tb.Text + "$"
        Next
        If colorto <> "" Then
            colorto = "colorto=" + colorto.TrimEnd("$") + ";"
            kkon = kkon + colorto
        End If
        ...
        For k As Integer = 1 To br
            ...
            If DirectCast(tb, TextBox).Text <> "" Then colorwhat = colorwhat + tb.Text + "$"
        Next
        If colorwhat <> "" Then
            colorwhat = "colorwhat=" + colorwhat.TrimEnd("$") + ";"
            kkon = kkon + colorwhat
        End If
