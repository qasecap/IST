...
Public Class mFormEnterEventReadOnly
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.Label50.Text = "ReadOnly If"
        '
        'mFormEnterEventReadOnly
        '
        ...
        Me.Name = "mFormEnterEventReadOnly"
...
#End Region
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.Label1.Text = "On Enter event on field  " + pp(2).Trim + "    Type: " + CStr(pp(5)).Trim + ", MaxLength= " + CStr(pp(6)).Trim + "    READONLY"
    End Sub
    ...
    Sub getMetaDataPane()
        ...
        For i As Integer = 0 To pp.GetUpperBound(0)
            ...
            If pp(i).Length > 9 AndAlso pp(i).Trim.ToUpper.IndexOf("READONLYIF") >= 0 OrElse pp(i).Length > 10 AndAlso pp(i).Trim.ToUpper.IndexOf("READONLYAKO") >= 0 Then
                readonlyif = pp(i).Trim.ToUpper.Replace("READONLYIF=", "").Replace("READONLYAKO=", "").Split("$")
            ElseIf pp(i).Length > 7 AndAlso pp(i).Trim.ToUpper.IndexOf("READONLY") >= 0 Then
                readonlyis = pp(i).Trim.ToUpper.Replace("READONLY=", "").Split("$")
            ElseIf pp(i).Trim <> "" Then
                enterEventOst = enterEventOst + pp(i) + ";"
            End If
        Next

        For i As Integer = 0 To readonlyif.GetUpperBound(0)
        ...
        For i As Integer = 0 To readonlyif.GetUpperBound(0)
            ...
            If Not tb Is Nothing Then DirectCast(tb, TextBox).Text = readonlyif(i).Trim
        Next

        For i As Integer = 0 To readonlyis.GetUpperBound(0)
            Dim tb As Control = NadjiKontrolu(Me, "tb" + CStr(i + 1).Trim)
            If Not tb Is Nothing Then DirectCast(tb, TextBox).Text = readonlyis(i).Trim
        Next
    ...
    Private Sub btn_Click(sender As Object, e As EventArgs) Handles btn.Click
        ...
        For k As Integer = 1 To 20
            ...
            If Not tb Is Nothing AndAlso DirectCast(tb, TextBox).Text <> "" Then
                readonlyif = readonlyif + tb.Text + "$"
                br = br + 1
            End If
        Next
        If readonlyif <> "" Then
            readonlyif = "readonlyif=" + readonlyif.TrimEnd("$") + ";"
            kkon = kkon + readonlyif
        End If
        ...
        For k As Integer = 1 To br
            ...
            If DirectCast(tb, TextBox).Text <> "" Then readonlyis = readonlyis + tb.Text + "$"
        Next
        If readonlyis <> "" Then
            readonlyis = "readonlyis=" + readonlyis.TrimEnd("$") + ";"
            kkon = kkon + readonlyis
        End If
