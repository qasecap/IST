...
#End Region
    ...
    Public Sub New(ByVal dIst As DatasetIstrazivanja, ByVal form As Form, ByVal tvRight As Integer, ByVal tvTop As Integer, ByVal sifist As String,
                   ByVal ISTConnectionString As String, ByVal DBConnectionString As String, ByVal whatVisible As String)
        InitializeComponent()
        _parentForm = form
        With Me
            '.TopMost = True
    ...
    Private Sub Form_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If whatVisible = "field" Then
            strSQl = "select polje as displaymember, polje  as valuemember" +
                    " from ISTPolja " +
                    " where sifist=N'" + sifist + "' and tabela=N'" + pp(0).Trim + "'" +
                    " and (izraz like N'%visiblefalse%' or vs='0' or izraz='$''''') order by rbr"
        Else
            strSQl = "select polje as displaymember, polje  as valuemember" +
                     " from ISTPolja " +
                     " where sifist=N'" + sifist + "' and tabela=N'" + pp(0).Trim + "'" +
                     " and (izraz like N'%bordernone%' or izraz='$''''' or izraz='''''') order by rbr"
        End If
        ...
        Try
            If whatVisible = "field" Then
                If Me.lbD.CheckedItems.Count > 0 Then
                    For i As Integer = 0 To Me.lbD.CheckedItems.Count - 1
                        strIST = strIST + "update ISTPolja set vs=case when vs='0' then '100' else vs end, izraz= case when izraz like " +
                                "N'%visiblefalse%' then replace(izraz,'visiblefalse','') when izraz like '$''''' then  '$''label''' else izraz end where sifist=N'" + sifist +
            ...
            Else
                If Me.lbD.CheckedItems.Count > 0 Then
                    For i As Integer = 0 To Me.lbD.CheckedItems.Count - 1
                        strIST = strIST + "update ISTPolja set vs=case when vs='0' then '100' else vs end, izraz= case when izraz like " +
                                "N'%bordernone%' then replace(izraz,'bordernone','') when izraz like '$''''' then  '$'' ''' when izraz like '''''' then  ''' ''' else izraz end where sifist=N'" + sifist +
                                "' and tabela=N'" + pp(0).Trim + "' and polje=N'" +
                                DirectCast((Me.lbD.CheckedItems(i)), System.Data.DataRowView).Row.ItemArray(0).ToString + "';" + nvrd
                    Next
                End If
